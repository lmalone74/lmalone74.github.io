---
title: Laura Malone
---

<p style="text-align: left; font-size: 18px;">
  <a href="/">Home</a> |
  <a href="/artifactshome.html">Artifacts</a> |
  <a href="/projects.html">Projects</a> |
  <a href="/contact.html">Contact</a>
</p>>

<table style="width: 100%; border: none;">
  <tr>
    <td style="width: 50%; vertical-align: middle; padding-right: 15px; border: none;">

  <div style="position: relative; width: 100%; padding-bottom: 56.25%; height: 0;">
        <iframe
          src="https://www.youtube.com/embed/NDn0UKiumF0"
          style="position: absolute; top: 0; left: 0; width: 100%; height: 100%;"
          frameborder="0"
          allowfullscreen>
        </iframe>
      </div>

  <p style="font-size: 13px; color: #666; margin-top: 8px;">
        If the video does not play,
        <a href="https://youtu.be/NDn0UKiumF0" target="_blank">watch it here</a>.
      </p>

  </td>

  <td style="width: 50%; vertical-align: middle; padding-left: 15px; border: none;">
      <h3>AWS Cloud Development Presentation</h3>
      <p>
        This presentation provides an overview of cloud-based application development using AWS, including serverless architecture, API Gateway, Lambda functions, and DynamoDB integration. It highlights how cloud technologies support scalability, modular design, and efficient resource management. The project demonstrates practical implementation of these services along with key considerations such as security roles, permissions, and real-world deployment challenges.
      </p>
    </td>
  </tr>
</table>

<table style="width: 100%; border: none;">
  <tr>
    <td style="width: 50%; vertical-align: middle; padding-right: 15px; border: none;">

  <div style="position: relative; width: 100%; padding-bottom: 56.25%; height: 0;">
        <iframe
          src="https://www.youtube.com/embed/1jCqHLq_8Jw"
          style="position: absolute; top: 0; left: 0; width: 100%; height: 100%;"
          frameborder="0"
          allowfullscreen>
        </iframe>
      </div>

  <p style="font-size: 13px; color: #666; margin-top: 8px;">
        If the video does not play,
        <a href="https://youtu.be/1jCqHLq_8Jw" target="_blank">watch it here</a>.
      </p>

  </td>

  <td style="width: 50%; vertical-align: middle; padding-left: 15px; border: none;">
      <h3>GAM-312 Unreal BP and C++ Code and Game Review</h3>
      <p>
       For GAM-312 at SNHU we were asked to create one level of a survival game, as we were not required to create all of our own assets, I did take advantage of the FAB Store, which allowed me to really explore the game mechanics more than 3d modelling.
      </p>
    </td>
  </tr>
</table>
